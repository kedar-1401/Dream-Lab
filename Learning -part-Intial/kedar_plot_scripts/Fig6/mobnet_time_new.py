...
ax1.bar(110,8.12,width=width,label='',align='center',color = '#fadc6f', zorder = 3)
...
ax1.vlines(x=vlines_x_between,ymin=0,ymax=80,linestyles='solid',color='darkgray',linewidth=1.5)
...
ax1.text(110, 70, '22.3h*', color='purple', fontsize=12, ha='center', va='center', rotation=90)
...
# ax1.text(s + 1.25, 10 + 0.5, str(local_time), color='black', ha='center')
...
data_collection_medians = [x/60 for x in data_collection_medians]
...
#plt.legend(handles=handles[-3:], labels=labels[-3:], loc='upper left') #,bbox_to_anchor=(0.5, 1)
plt.legend(handles=handles[-3:], labels=labels[-3:], loc='upper left',bbox_to_anchor=(0.1, 1))

# Show the plot 
plt.tight_layout()

plt.savefig(f'./mobnet_time_new.png', dpi=600, bbox_inches='tight')
plt.savefig(f'./mobnet_time_new.pdf',dpi=300,bbox_inches='tight')
