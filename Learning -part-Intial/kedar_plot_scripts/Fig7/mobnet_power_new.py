...
# data3 = pd.read_csv(r'C:\Users\saisa\Documents\GitHub\sigmetrics2024\plot_scripts\Fig8\data\nn_modeling\NN_transfer_resnet_mobnet_MAPE_all.csv')
...
ax1.bar(110,3.62,width=width,label='',align='center',color = '#fadc6f', zorder = 3)
...
ax1.text(110, 26, '22.3h*', color='purple', fontsize=12, ha='center', va='center', rotation=90)

#ax3 = ax1.twinx()
# ax1.axvline(x=115, color='black', linestyle='-', linewidth=1.5)


# #remove the tick label for x axis
# ax1.set_yticklabels([])
# #put ticks and tick labels on the axvline
# ax1.set_yticks([])
...
# #for the two bars representing global and local power print the values on the top of the bar
# ax1.text(s - 1.25, 25 + 0.5, str(global_power), color='black', ha='center')
# ax1.text(s + 1.25, 20 + 0.5, str(local_power), color='black', ha='center')
...
#index_df = pd.read_csv(r'C:\Users\saisa\Documents\GitHub\sigmetrics2024\plot_scripts\Fig8\data\mobnet_v4_runs_merged\minibatch_index_list_mobilenet_new.csv')
index_df = pd.read_csv(r'data\mobnet_v4_runs_merged\minibatch_index_list_mobilenet_new.csv')
...
plt.legend(handles=handles[-3:], labels=labels[-3:], loc='upper center') #,bbox_to_anchor=(0.5, 1)
# Show the plot
plt.tight_layout()

plt.savefig(f'./mobnet_power_new.png', dpi=600, bbox_inches='tight')
plt.savefig(f'./mobnet_power_new.pdf',dpi=600,bbox_inches='tight')
