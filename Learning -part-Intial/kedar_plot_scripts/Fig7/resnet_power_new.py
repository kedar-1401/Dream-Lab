...
#data2 = pd.read_csv(r'C:\Users\saisa\Documents\GitHub\sigmetrics2024\plot_scripts\Fig8\data\nn_modeling//NN_resnet_MAPE_all.csv')
data2 = pd.read_csv(r'data\nn_modeling//NN_resnet_MAPE_all.csv')

#data3 = pd.read_csv(r'C:\Users\beaut\Downloads\barplots\data\nn_modeling\NN_transfer_mobnet_resnet_MAPE.csv')
...
# Group the data and calculate the median of 'Val_MAPE_power'
# grouped_data1_all = data1.groupby('No_powermodes')['Val_MAPE_power'].apply(list)
# grouped_data2_all = data2.groupby('No_powermodes')['Val_MAPE_power'].apply(list)
# grouped_data3_all = data3.groupby('No_powermodes')['Val_MAPE_power'].apply(list)
...
ax1.bar(110,4.06,width=width,label='',align='center',color = '#fadc6f', zorder = 3)
ax1.set_xlabel('# Power modes used for Training')
ax1.set_ylabel('Power MAPE (%)', color='black')
#ax1.set_title('Median Val_MAPE_power and Different No_powermodes Groups')
...
ax1.text(110, 36, '14.8h*', color='purple', fontsize=12, ha='center', va='center', rotation=90)
...
# ax1.text(s - 1.25, global_power + 0.5, str(global_power), color='black', ha='center')
# ax1.text(s + 1.25, local_power + 0.5, str(local_power), color='black', ha='center')

# ax1.bar(s - 1.25, global_power, color='#fb9a3c', width=width)

# ax1.bar(s + 1.25, local_power, color='#fcc28a', width=width)
...
# index_df = pd.read_csv(r'C:\Users\saisa\Documents\GitHub\sigmetrics2024\plot_scripts\Fig8\data\resnet_v4_runs_merged//minibatch_index_list_resnet_new.csv')
index_df = pd.read_csv(r'data\resnet_v4_runs_merged//minibatch_index_list_resnet_new.csv')
...
plt.legend(handles=handles[-2:], labels=labels[-2:], loc='upper left')#,bbox_to_anchor=(0.1, 1))

# Show the plot
plt.tight_layout()

plt.savefig(f'./resnet_power_new.png', dpi=600, bbox_inches='tight')
plt.savefig(f'./resnet_power_new.pdf',dpi=300,bbox_inches='tight')
