...
ax1.bar(110,4.81,width=width,label='',align='center',color = '#fadc6f', zorder = 3)
...
ax1.text(110, 26, '29.7h*', color='purple', fontsize=12, ha='center', va='center', rotation=90)
...
# ax1.text(s - 1.25, 28 + 0.5, str(global_power), color='black', ha='center')
# ax1.text(s + 1.25, 26 - 0.5, str(local_power), color='black', ha='center')
...
#index_df = pd.read_csv(r'C:\Users\saisa\Documents\GitHub\sigmetrics2024\plot_scripts\Fig8\data\yolo_runs\minibatch_index_list_yolo_new.csv')
...
plt.legend(handles=handles[-3:], labels=labels[-3:], loc='upper left',prop={'size': 10},bbox_to_anchor=(0.1, 1))

# Show the plot
plt.tight_layout()

plt.savefig(f'./yolo_power_new.png', dpi=600, bbox_inches='tight')
plt.savefig(f'./yolo_power_new.pdf',dpi=300,bbox_inches='tight')
