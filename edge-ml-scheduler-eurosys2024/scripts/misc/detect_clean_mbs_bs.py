...
def generate_index_list(all_powermodes,path,powerstats_filename,mb_filename, model):
    pm_count = 0
    short_rt=0
    undetected_count=0
    window_size = 10
    ...
    for powermode in all_powermodes:
        ...
        mb_file = path + "/" + "pm_" + powermode + "/" + mb_filename
        minibatch_df = pd.read_csv(mb_file)
        powerstats_df = pd.read_csv(powerstats_file)
        ...
        if model == "lstm":
            start_time = powerstats_df['log_time'].iloc[-1] - 0.5
            end_time = powerstats_df['log_time'].iloc[-1] + 1
            clean_minibatches = 1
            # skip_index = len(minibatch_df) - 1  # Placeholder value for skip_index
            skip_index = -1
            minibatch_index_list.append([cores, cpu, gpu, mem, skip_index, start_time, end_time, clean_minibatches])
    ...
    #convert to csv
    minibatch_index_list.to_csv("minibatch_index_list_lstm_bs1.csv", index=False)

path="lstm_infer_bs_runs/bs_1"
...
#print(len(all_powermodes))
generate_index_list(all_powermodes,path,powerstats_filename,mb_filename, model)
